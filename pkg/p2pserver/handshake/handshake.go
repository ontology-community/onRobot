...
func HandshakeClient(info *peer.PeerInfo, selfId *common.PeerKeyId, conn net.Conn) (*peer.PeerInfo, error) {
	...
	// mark:
	if params.ValidateHandshakeStopLevel(params.Handshake_StopClientAfterSendVersion) {
		return nil, fmt.Errorf("client handshake stopped after send version")
	}

	// mark:
	if params.HandshakeClientTimeout > 0 {
		time.Sleep(params.HandshakeClientTimeout)
	}
	...
	// mark:
	if params.ValidateHandshakeStopLevel(params.Handshake_StopClientAfterReceiveVersion) {
	...
	if useDHT(receivedVersion.P.SoftVersion, info.SoftVersion) {
		...
		// mark:
		if params.ValidateHandshakeStopLevel(params.Handshake_StopClientAfterUpdateKad) {
		...
		// mark:
		if params.ValidateHandshakeStopLevel(params.Handshake_StopClientAfterReadKad) {
	...
	// mark:
	if params.ValidateHandshakeStopLevel(params.Handshake_StopClientAfterSendAck) {
...
func HandshakeServer(info *peer.PeerInfo, selfId *common.PeerKeyId, conn net.Conn) (*peer.PeerInfo, error) {
	...
	// mark:
	if params.HandshakeServerTimeout > 0 {
		time.Sleep(params.HandshakeServerTimeout)
	}
	...
	// mark:
	if params.ValidateHandshakeStopLevel(params.Handshake_StopServerAfterReceiveVersion) {
	...
	// mark:
	if params.ValidateHandshakeStopLevel(params.Handshake_StopServerAfterSendVersion) {
	...
	if useDHT(version.P.SoftVersion, info.SoftVersion) {
		...
		kid = kadkeyId.KadKeyId.Id
		// mark:
		if params.ValidateHandshakeStopLevel(params.Handshake_StopServerAfterReadKad) {
		...
		// mark:
		if params.ValidateHandshakeStopLevel(params.Handshake_StopServerAfterUpdateKad) {
	...
	// mark:
	if params.ValidateHandshakeStopLevel(params.Handshake_StopServerAfterReadAck) {
