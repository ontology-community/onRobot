...
import (
	...
	"github.com/ontology-community/onRobot/pkg/p2pserver/dht"
...
func (self *Discovery) FindNodeHandle(ctx *p2p.Context, freq *types.FindNodeReq) {
	...
	if !self.maskSet.Has(remoteIP.String()) && (self.maskFilter == nil ||
		!self.maskFilter.Contains(remotePeer.Info.RemoteListenAddress())) {
		...
		for _, pair := range fresp.CloserPeers {
			...
			// hide mask node
			if self.maskSet.Has(ip) || (self.maskFilter != nil && self.maskFilter.Contains(pair.Address)) {
				continue
			}
