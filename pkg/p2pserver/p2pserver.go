...
func NewServer(acct *account.Account) (*P2PServer, error) {
	...
	protocol := protocols.NewMsgHandler(acct, connect_controller.NewStaticReserveFilter(recRsv), db)
...
func NewStatServer(protocol p2p.Protocol, conf *config.P2PNodeConfig) (*P2PServer, error) {
	n, err := netserver.NewNetServerWithTxStat(protocol, conf, nil)
	if err != nil {
		return nil, err
	}
