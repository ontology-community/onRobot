...
func isBalanceEnough(address common.Address, gas uint64) bool {
	...
	if err != nil {
		log4.Debug("failed to get contract balance %s err %v",
...
func replyTxResult(txResultCh chan *tc.TxResult, hash common.Uint256,
	...
	default:
		log4.Debug("handleTransaction: duplicated result")
	}
}
...
func preExecCheck(txn *tx.Transaction) (bool, string) {
	...
	if err != nil {
		log4.Debug("preExecCheck: failed to preExecuteContract tx %x err %v",
			txn.Hash(), err)
	}
	if txn.GasLimit < result.Gas {
		log4.Debug("preExecCheck: transaction's gasLimit %d is less than preExec gasLimit %d",
	...
	if overflow {
		log4.Debug("preExecCheck: gasPrice %d preExec gasLimit %d overflow",
			txn.GasPrice, result.Gas)
		return false, fmt.Sprintf("gasPrice %d preExec gasLimit %d overflow",
			txn.GasPrice, result.Gas)
	}
	if !isBalanceEnough(txn.Payer, gas) {
		log4.Debug("preExecCheck: transactor %s has no balance enough to cover gas cost %d",
...
func (ta *TxActor) handleTransaction(sender tc.SenderType, self *actor.PID,
	...
	if len(txn.ToArray()) > tc.MAX_TX_SIZE {
		log4.Debug("handleTransaction: reject a transaction due to size over 1M")
	...
	if ta.server.getTransaction(txn.Hash()) != nil {
		log4.Debug("handleTransaction: transaction %x already in the txn pool",
			txn.Hash())
		...
		log4.Debug("handleTransaction: transaction pool is full for tx %x",
			txn.Hash())
		...
		if _, overflow := common.SafeMul(txn.GasLimit, txn.GasPrice); overflow {
			log4.Debug("handleTransaction: gasLimit %v, gasPrice %v overflow",
		...
		if txn.GasLimit < gasLimitConfig || txn.GasPrice < gasPriceConfig {
			log4.Debug("handleTransaction: invalid gasLimit %v, gasPrice %v",
				txn.GasLimit, txn.GasPrice)
			if sender == tc.HttpSender && txResultCh != nil {
				replyTxResult(txResultCh, txn.Hash(), errors.ErrUnknown,
		...
		if txn.TxType == tx.Deploy && txn.GasLimit < neovm.CONTRACT_CREATE_GAS {
			log4.Debug("handleTransaction: deploy tx invalid gasLimit %v, gasPrice %v",
				txn.GasLimit, txn.GasPrice)
			if sender == tc.HttpSender && txResultCh != nil {
				replyTxResult(txResultCh, txn.Hash(), errors.ErrUnknown,
		...
		if !ta.server.disablePreExec {
			if ok, desc := preExecCheck(txn); !ok {
				log4.Debug("handleTransaction: preExecCheck tx %x failed", txn.Hash())
				if sender == tc.HttpSender && txResultCh != nil {
					replyTxResult(txResultCh, txn.Hash(), errors.ErrUnknown, desc)
				}
				return
			}
			log4.Debug("handleTransaction: preExecCheck tx %x passed", txn.Hash())
		}
		<-ta.server.slots
...
func (ta *TxActor) Receive(context actor.Context) {
	...
	case *tc.GetTxnReq:
		...
		log4.Debug("txpool-tx actor receives getting tx req from %v", sender)
	...
	case *tc.GetTxnStats:
		...
		log4.Debug("txpool-tx actor receives getting tx stats from %v", sender)
	...
	case *tc.CheckTxnReq:
		...
		log4.Debug("txpool-tx actor receives checking tx req from %v", sender)
	...
	case *tc.GetTxnStatusReq:
		...
		log4.Debug("txpool-tx actor receives getting tx status req from %v", sender)
	...
	case *tc.GetTxnCountReq:
		...
		log4.Debug("txpool-tx actor receives getting tx count req from %v", sender)
...
func (tpa *TxPoolActor) Receive(context actor.Context) {
	switch msg := context.Message().(type) {
	case *actor.Started:
		log4.Info("txpool actor started and be ready to receive txPool msg")

	case *actor.Stopping:
		log4.Warn("txpool actor stopping")

	case *actor.Restarting:
		log4.Warn("txpool actor Restarting")

	case *tc.GetTxnPoolReq:
		sender := context.Sender()

		log4.Debug("txpool actor receives getting tx pool req from %v", sender)
	...
	case *tc.GetPendingTxnReq:
		...
		log4.Debug("txpool actor receives getting pedning tx req from %v", sender)
	...
	case *tc.VerifyBlockReq:
		...
		log4.Debug("txpool actor receives verifying block req from %v", sender)
	...
	case *message.SaveBlockCompleteMsg:
		...
		log4.Debug("txpool actor receives block complete event from %v", sender)
	...
	default:
		log4.Debug("txpool actor: unknown msg %v type %v", msg, reflect.TypeOf(msg))
	}
}
...
func (vpa *VerifyRspActor) Receive(context actor.Context) {
	switch msg := context.Message().(type) {
	case *actor.Started:
		log4.Info("txpool-verify actor: started and be ready to receive validator's msg")

	case *actor.Stopping:
		log4.Warn("txpool-verify actor: stopping")

	case *actor.Restarting:
		log4.Warn("txpool-verify actor: Restarting")

	case *types.RegisterValidator:
		log4.Debug("txpool-verify actor:: validator %v connected", msg.Sender)
		vpa.server.registerValidator(msg)

	case *types.UnRegisterValidator:
		log4.Debug("txpool-verify actor:: validator %d:%v disconnected", msg.Type, msg.Id)

		vpa.server.unRegisterValidator(msg.Type, msg.Id)

	case *types.CheckResponse:
		log4.Debug("txpool-verify actor:: Receives verify rsp message")

		vpa.server.assignRspToWorker(msg)

	default:
		log4.Debug("txpool-verify actor:Unknown msg %v type %v", msg, reflect.TypeOf(msg))
	}
}
